...
static void print_report(int is_last_report, int64_t timer_start, int64_t cur_time)
{
    ...
    ffcmd_notify_msg2(&g_cmd->msg_queue, FFCMD_INFO_PROGRESS, (int)(mss*1000));
...
static int transcode(void)
{
    ...
    while (!received_sigterm) {
        if(g_cmd->abort) {
            av_log(NULL, AV_LOG_INFO, "g_cmd->abort is true, ffmpeg stop and exit\n");
            break;
        }
...
int ffmpeg_main(int argc, char **argv, XMFFmpegCmd *cmd)
{
    int ret;
    int64_t ti;
    g_cmd = cmd;
...
//     }
    ...
    if (transcode() < 0) {
        exit_program(1);
        av_log(NULL, AV_LOG_INFO, "transcode error or stop, exit\n");
        ffmpeg_cleanup(0);
        return -1;
    }
    ti = getutime() - ti;
